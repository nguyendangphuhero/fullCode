...
class AuthController{
    ...
    getAllUser = async (req:Request, res:Response, next: NextFunction):Promise<any> =>{
        try{
            ...
            return next(new AppError('Bi loi roi', INTERNAL_SERVER_ERROR));
        }
    }
    ...
    protectRoute =  async(req:Request, res:Response,next:NextFunction): Promise<any> =>{
        ...
        //console.log("token vo product",token)
            
        
        if(!token) return next( new AppError(JWT_TOKEN_NOT_FOUND, BAD_REQUEST) );

        const {id, error} =  tokenUtils.decodeJwt(token);
        //console.log("id ben cho product",id)
